...
class graph_Thread(QThread):
    ...
    def get_keypoints(self, json_file):
        f = open(json_file,)
        data = json.load(f)
    ...
    def drawline_from_point_list(self, image,points,joint_name):
        '''
    ...
    def colouring_halt_points(self,image,points):
        # Blue color in RGB
        color = blue
        count=0
        thresh=5
    ...
    def generate_frame_plot(self, json_files_location, output_location):
        global video_frames
        video_no=0
        per_frame=1
        ...
        for video_frames_folder in os.listdir(json_files_location):
            ...
            for frame_file in os.listdir(single_video_frames_path):
                ...
                #getting keypoints
                    try:
                        all_keypoints= self.get_keypoints(os.path.join(single_video_frames_path,frame_file))
            ...
            for i in range (0,len(video_frames_header),2):
                ...
                #print (points)
                img = self.drawline_from_point_list(img,points,video_frames_header[i])
                img = self.colouring_halt_points(img,points)
                #calc_vel_acc(points)
                #detect_dir(points)
    ...
    def run(self):
        ...
        files = glob.glob('image_plot/arm_movement_good/*')
        for f in files:
            os.remove(f)

        files = glob.glob('image_plot/arm_movement_bad/*')
        for f in files:
            os.remove(f)
        ...
        for f in files:
            os.remove(f)
        '''                       
        generate_frame_plot('D:/Elderly Assistant External files/Exercise Videos/Openpose variations/openpose-1.7.0-binaries-win64-gpu/openpose/python/output_json_folder/arm_movement_good/','image_plot/arm_movement_good/')
        generate_frame_plot('D:/Elderly Assistant External files/Exercise Videos/Openpose variations/openpose-1.7.0-binaries-win64-gpu/openpose/python/output_json_folder/arm_movement_bad/','image_plot/arm_movement_bad/')
        
        '''
        self.generate_frame_plot('movenet/live_json/',
                            'image_plot/live_record/')
